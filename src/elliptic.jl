...
@inline function D_ellipk(m::T, Km::T, Em::T) where {T<:Real}
    m == 0 && return 9π / 64.0
    return ((Em / (1-m)) - Km) / (2m)
end

D2_ellipk(m) = m == 0 ?  π / 8.0 : D2_ellipk(m, ellipke(m)...)
@inline function D2_ellipk(m::T, Km::T, Em::T, dKm=D_ellipk(m, Km, Em), dEm=D_ellipe(m, Km, Em)) where {T<:Real}
    m == 0 && return π / 8.0
    inv1_m = 1.0 / (1.0 - m)
    E_1_m = Em * inv1_m
...
@doc raw"""
    ellipke_nonneg(m) -> (Km, Em)

Compute complete elliptic integrals K(m) and E(m) for non-negative m in a single pass.
Assumes 0 ≤ m < 1. For negative m, use `ellipke(m)` instead.

# Example
```julia
m = 0.5
Km, Em = ellipke_nonneg(m)
```

See also: [`ellipk_nonneg`](@ref), [`ellipe_nonneg`](@ref), [`ellipke`](@ref)
"""
@inline function ellipke_nonneg(m::T) where {T<:Real}
    x = m

    # Fast edge cases
    x == 0.0 && return (pi / 2, pi / 2)
    x == 1.0 && return (Inf, 1.0)

    if x > 1.0
        x ≈ 1.0 && return (Inf, 1.0)
        throw(DomainError(m, "`m` must lie in [0, 1) for ellipke_nonneg_v6"))
    end

    # Compute index once
    idx = unsafe_trunc(Int, x * 10)

    # Binary tree structure: First split at idx < 4
    if idx < 4  # Tables 2-5 (0.0 <= x < 0.4)
        if idx < 2  # Tables 2-3 (0.0 <= x < 0.2)
            if idx == 0  # 0.0 <= x < 0.1 - Table 2
                t = x - 0.05
                km = @horner(t,
                    1.591003453790792180, 0.416000743991786912, 0.245791514264103415,
                    0.179481482914906162, 0.144556057087555150, 0.123200993312427711,
        ...
        else  # idx >= 2: Tables 4-5 (0.2 <= x < 0.4)
            if idx == 2  # 0.2 <= x < 0.3 - Table 4
                t = x - 0.25
                km = @horner(t,
                    1.685750354812596043, 0.541731848613280329, 0.401524438390690257,
                    0.369642473420889090, 0.376060715354583645, 0.405235887085125919,
    ...
    else  # idx >= 4: Tables 6-9 and special cases (0.4 <= x < 1.0)
        if idx < 6  # Tables 6-7 (0.4 <= x < 0.6)
            if idx == 4  # 0.4 <= x < 0.5 - Table 6
                t = x - 0.45
                km = @horner(t,
                    1.813883936816982644, 0.763163245700557246, 0.761928605321595831,
                    0.951074653668427927, 1.315180671703161215, 1.928560693477410941,
        ...
        else  # idx >= 6: Tables 8-9 and special (0.6 <= x < 1.0)
            if idx < 8  # Tables 8-9 (0.6 <= x < 0.8)
                if idx == 6  # 0.6 <= x < 0.7 - Table 8
                    t = x - 0.65
                    km = @horner(t,
                        2.007598398424376302, 1.248457231212347337, 1.926234657076479729,
                        3.751289640087587680, 8.119944554932045802, 18.66572130873555361,
            ...
            elseif idx == 8  # 0.8 <= x < 0.9 (Tables 10-11)
                # Need one float comparison to distinguish
                if x < 0.85  # Table 10
                    t = x - 0.825
                    km = @horner(t,
                        2.318122621712510589, 2.616920150291232841, 7.897935075731355823,
            ...
            else  # idx >= 9: x >= 0.9 - Special handling
                td = 1.0 - x
                td1 = td - 0.05
...
Compute K(m) for non-negative m.

Equivalent to `ellipke_nonneg(m)[1]`. For both K and E, use
`ellipke_nonneg` to avoid redundant computation.

See also: [`ellipke_nonneg`](@ref)
"""
@inline ellipk_nonneg(m::Real) = ellipke_nonneg(m)[1]


@doc raw"""
    ellipe_nonneg(m) -> Em

Compute E(m) for non-negative m.

Equivalent to `ellipke_nonneg(m)[2]`. For both K and E, use
`ellipke_nonneg` to avoid redundant computation.

See also: [`ellipke_nonneg`](@ref)
"""
@inline ellipe_nonneg(m::Real) = ellipke_nonneg(m)[2]

@doc raw"""
    ellipke(m) -> (Km, Em)

Compute both complete elliptic integrals K(m) and E(m) for any m ∈ (-∞, 1].

Uses `ellipke_nonneg` for efficient computation:
- If m ≥ 0: Directly computes using `ellipke_nonneg(m)`
- If m < 0: Transforms to positive domain using the relations:
  ```math
  x = \frac{m}{m-1}, \quad K(m) = \frac{K(x)}{\sqrt{1-m}}, \quad E(m) = E(x) \sqrt{1-m}
  ```
...
See also: [`ellipke_nonneg`](@ref), [`ellipk`](@ref), [`ellipe`](@ref)
"""
@inline function ellipke(m::T) where T<:Real
    if m >= zero(T)
        # Non-negative m: use fast path directly
        return ellipke_nonneg(m)
    else
        # Negative m: transform to positive domain
        x = m / (m - one(T))              # x > 0 when m < 0
        sqrt_factor = sqrt(one(T) - m)    # sqrt(1 - m) > 1 when m < 0
        Kx, Ex = ellipke_nonneg(x)
